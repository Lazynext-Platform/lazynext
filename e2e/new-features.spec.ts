...
test.describe('New feature pages', () => {
  ...
  test('/google-safety page loads', async ({ page }) => {
    const res = await page.goto('/google-safety');
    expect(res).not.toBeNull();
    expect([200, 401, 403]).toContain(res!.status());
    await page.waitForLoadState('networkidle').catch(() => {});
    await expect(page.locator('body')).toBeVisible();
  ...
  test('/performance-loop page loads', async ({ page }) => {
    const res = await page.goto('/performance-loop');
    expect(res).not.toBeNull();
    expect([200, 401, 403]).toContain(res!.status());
    await page.waitForLoadState('networkidle').catch(() => {});
    await expect(page.locator('body')).toBeVisible();
  ...
  test('/viral-analyzer page loads', async ({ page }) => {
    const res = await page.goto('/viral-analyzer');
    expect(res).not.toBeNull();
    expect([200, 401, 403]).toContain(res!.status());
    await page.waitForLoadState('networkidle').catch(() => {});
    await expect(page.locator('body')).toBeVisible();
  ...
  test('/skill-chains page loads', async ({ page }) => {
    const res = await page.goto('/skill-chains');
    expect(res).not.toBeNull();
    expect([200, 401, 403]).toContain(res!.status());
    await page.waitForLoadState('networkidle').catch(() => {});
    await expect(page.locator('body')).toBeVisible();
