...
export async function GET(req: NextRequest) {
  ...
  const audits = await AssetTrackingService.listAudits(organizationId, opts as never);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const audit = await AssetTrackingService.createAudit(ws.organizationId, ws.id, {
      ...
      scope: body.scope, findings: body.findings, discrepancies: body.discrepancies, notes: body.notes,
    }, session.user.id);
    return NextResponse.json({ audit }, { status: 201 });
  } catch (e) {
    console.error('[asset-tracking/audits] create error:', e);
