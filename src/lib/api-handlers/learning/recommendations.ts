...
/**
 * GET /api/learning/recommendations — get learning-based recommendations.
 * Query: workspaceId, organizationId
 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const recommendations = await LearningLoopService.getRecommendations(workspaceId, organizationId);

    return NextResponse.json({ recommendations });
  } catch (e) {
    console.error('[learning/recommendations] error:', e);
    return NextResponse.json({ error: 'failed_to_get_recommendations' }, { status: 500 });
  }
}
