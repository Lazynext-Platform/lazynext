...
function extractJson(raw: string): Record<string, unknown> {
  ...
  if (a < 0 || b < 0) throw new Error('no_json_in_brand_voice_consistency_checker_output');
...
function parseCheckJson(
  ...
  const check: VoiceConsistencyCheck = {
    ...
    recommendations: asStrArray(checkRaw.recommendations),
  };
...
export async function checkBrandVoiceConsistency(
  ...
  try {
    const raw = await atlasChat(
      [{ role: 'system', content: BRAND_VOICE_CONSISTENCY_CHECKER_SYS }, { role: 'user', content: userPrompt }],
