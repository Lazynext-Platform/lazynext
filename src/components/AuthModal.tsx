...
export function AuthModal({ open, onClose, initialMode = 'signin' }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#1a1b1e] p-6 shadow-2xl"
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {mode === 'signup' && (
            <input
              ...
              className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-neutral-500 outline-none transition focus:border-brand-400"
            />
          )}
          <input
            type="email"
