...
interface MetaSafetyState {
  config?: SafetyConfig;
  auditSummary?: AuditSummary;
  pendingApprovals?: PendingApproval[];
}

export function MetaSafetyDashboard() {
  const { t } = useI18n();
  ...
  const [state, setState] = useState<MetaSafetyState | null>(null);
  ...
  const loadState = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/ads/meta-safety');
  ...
  }, [dryRun, requireApproval, maxDailyBudget, maxCampaignBudget, maxDailyMutations, warningThreshold, allowedActions, blockedActions, loadState]);

  const handleApproval = useCallback(async (id: string, approved: boolean) => {
    setApprovingId(id);
    setError('');
    try {
      const res = await fetch('/api/ads/meta-approve', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold flex items-center gap-2"><Shield className="w-5 h-5" /> {t('metaSafety.title')}</h2>
        <p className="text-sm text-fg-muted mt-1">{t('metaSafety.subtitle')}</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <label className="flex items-center gap-2 rounded-lg border border-border bg-bg-card px-3 py-2 text-sm">
            ...
            {t('metaSafety.requireApproval')}
          </label>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label htmlFor="msMaxDailyBudget" className="block text-sm font-medium mb-1">{t('metaSafety.maxDailyBudget')}</label>
            <input id="msMaxDailyBudget" type="number" value={maxDailyBudget} onChange={(e) => setMaxDailyBudget(e.target.value)} className="w-full rounded-lg border border-border bg-bg-card px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent" disabled={saving} />
          </div>
          <div>
            <label htmlFor="msMaxCampaignBudget" className="block text-sm font-medium mb-1">{t('metaSafety.maxCampaignBudget')}</label>
            <input id="msMaxCampaignBudget" type="number" value={maxCampaignBudget} onChange={(e) => setMaxCampaignBudget(e.target.value)} className="w-full rounded-lg border border-border bg-bg-card px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent" disabled={saving} />
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label htmlFor="msMaxDailyMutations" className="block text-sm font-medium mb-1">{t('metaSafety.maxDailyMutations')}</label>
            <input id="msMaxDailyMutations" type="number" value={maxDailyMutations} onChange={(e) => setMaxDailyMutations(e.target.value)} className="w-full rounded-lg border border-border bg-bg-card px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent" disabled={saving} />
          </div>
          <div>
            <label htmlFor="msWarningThreshold" className="block text-sm font-medium mb-1">{t('metaSafety.warningThreshold')}</label>
            <input id="msWarningThreshold" type="number" min={0} max={100} value={warningThreshold} onChange={(e) => setWarningThreshold(e.target.value)} className="w-full rounded-lg border border-border bg-bg-card px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent" disabled={saving} />
          </div>
        </div>
        ...
        <button onClick={save} disabled={saving} className="rounded-lg bg-brand-accent px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50 flex items-center gap-2">
          ...
          {saving ? t('metaSafety.saving') : t('metaSafety.save')}
        </button>
      </div>
      ...
      {audit && (
        <div className="rounded-lg border border-border bg-bg-card p-4">
          <h3 className="font-medium flex items-center gap-2 mb-3"><Shield className="w-4 h-4" /> {t('metaSafety.auditLog')}</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="text-center">
              <div className="text-xs text-fg-muted">Successes</div>
              <div className="text-lg font-bold text-success">{audit.successes ?? 0}</div>
            </div>
            <div className="text-center">
              <div className="text-xs text-fg-muted">Failures</div>
              <div className="text-lg font-bold text-danger">{audit.failures ?? 0}</div>
            </div>
            <div className="text-center">
              <div className="text-xs text-fg-muted">Simulated</div>
      ...
      {state?.pendingApprovals && state.pendingApprovals.length > 0 && (
        <div className="rounded-lg border border-border bg-bg-card p-4">
          <h3 className="font-medium flex items-center gap-2 mb-3"><Clock className="w-4 h-4" /> {t('metaSafety.pendingApprovals')} ({state.pendingApprovals.length})</h3>
              ...
              <div key={p.id || i} className="flex items-center justify-between border border-border rounded-lg p-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => p.id && handleApproval(p.id, true)} disabled={approvingId === p.id} className="rounded-lg bg-success px-3 py-1.5 text-xs font-medium text-white hover:opacity-90 disabled:opacity-50 flex items-center gap-1">
                    <Check className="w-3 h-3" /> {t('metaSafety.approve')}
                  </button>
                  <button onClick={() => p.id && handleApproval(p.id, false)} disabled={approvingId === p.id} className="rounded-lg bg-danger px-3 py-1.5 text-xs font-medium text-white hover:opacity-90 disabled:opacity-50 flex items-center gap-1">
                    <X className="w-3 h-3" /> {t('metaSafety.reject')}
                  </button>
                </div>
              </div>
            ))}
      ...
      {loadingState && <div className="flex items-center gap-2 text-sm text-fg-muted"><Loader2 className="w-4 h-4 animate-spin" /> Loading...</div>}
    </div>
  );
}
