  ...
  return (
    <div className="flex items-center justify-center py-16">
      <div className="max-w-md text-center px-6">
        ...
        <p className="mt-2 text-sm text-fg-secondary">{error.message || 'Failed to load task.'}</p>
        <div className="mt-6 flex items-center justify-center gap-4">
          <button onClick={reset} className="text-sm text-fg-secondary hover:text-fg transition">Try again</button>
          <a href="/tasks" className="text-sm text-fg-secondary hover:text-fg transition">Back to tasks</a>
        </div>
      </div>
    </div>
  );
