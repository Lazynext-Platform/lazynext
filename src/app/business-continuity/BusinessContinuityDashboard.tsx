...
import {
  Activity, ClipboardList, RefreshCw, FileBarChart, TestTube, BarChart3,
  Search,
} from 'lucide-react';
import { Card, Badge, Button, EmptyState } from '@/components/ui';
import type {
  ContinuityPlan, RecoveryStrategy, BusinessImpactAnalysis, ContinuityTest,
  BusinessContinuityMetrics, BusinessContinuityStats,
} from '@/lib/services/business-continuity-service';

type TabId = 'overview' | 'plans' | 'strategies' | 'bias' | 'tests';
...
interface BusinessContinuityDashboardProps {
  ...
  metrics: BusinessContinuityMetrics;
  stats: BusinessContinuityStats;
}
...
export function BusinessContinuityDashboard({
  plans, strategies, bias, tests, metrics, stats,
  ...
  const tabs: { id: TabId; label: string; icon: typeof Activity }[] = [
    ...
    { id: 'strategies', label: 'Strategies', icon: RefreshCw },
    { id: 'bias', label: 'BIAs', icon: FileBarChart },
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
            ...
            <Card className="p-4">
              <div className="text-xs text-fg-tertiary">Implemented Strategies</div>
              <div className="mt-1 text-2xl font-bold">{metrics.implementedStrategies}</div>
            </Card>
            <Card className="p-4">
              <div className="text-xs text-fg-tertiary">Completed BIAs</div>
              <div className="mt-1 text-2xl font-bold">{metrics.completedBIAs}</div>
            </Card>
            <Card className="p-4">
          ...
          <Card className="p-6">
            <h3 className="mb-4 text-sm font-semibold">Strategy Status Breakdown</h3>
            <div className="flex flex-wrap gap-2">
              {Object.entries(stats.byStrategyStatus).map(([status, count]) => (
                <Badge key={status} variant={statusVariant(status)}>{status.replace('_', ' ')}: {count}</Badge>
              ))}
            </div>
          </Card>
      ...
      {tab === 'plans' && (
        <div className="space-y-3">
          {filteredPlans.length === 0 ? (
            <Card className="p-8"><EmptyState icon={ClipboardList} title="No continuity plans" description="Continuity plans will appear here." /></Card>
          ) : (
            filteredPlans.map((p) => (
      ...
      {tab === 'strategies' && (
        <div className="space-y-3">
          {filteredStrategies.length === 0 ? (
            <Card className="p-8"><EmptyState icon={RefreshCw} title="No recovery strategies" description="Recovery strategies will appear here." /></Card>
          ) : (
              ...
              <Card key={s.id} className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-fg-secondary">{s.type.replace('_', ' ')} · {s.strategy || 'No strategy'}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {s.cost && <Badge variant="default">{s.cost}</Badge>}
      ...
      {tab === 'bias' && (
        <div className="space-y-3">
          {filteredBIAs.length === 0 ? (
            <Card className="p-8"><EmptyState icon={FileBarChart} title="No business impact analyses" description="Business impact analyses will appear here." /></Card>
          ) : (
            filteredBIAs.map((b) => (
              <Card key={b.id} className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-fg-secondary">{b.type.replace('_', ' ')} · {b.processName || 'No process'}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {b.criticality && <Badge variant="default">{b.criticality}</Badge>}
      ...
      {tab === 'tests' && (
        <div className="space-y-3">
          {filteredTests.length === 0 ? (
            <Card className="p-8"><EmptyState icon={TestTube} title="No continuity tests" description="Continuity tests will appear here." /></Card>
          ) : (
            filteredTests.map((t) => (
              <Card key={t.id} className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {t.participants > 0 && <Badge variant="default">{t.participants} participants</Badge>}
