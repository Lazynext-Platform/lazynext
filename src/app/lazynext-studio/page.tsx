          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-stretch">
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              ...
              <div className="flex items-center gap-2 flex-wrap mt-2">
                <select value={formatId} onChange={(e) => setFormatId(e.target.value)} className={selCls} title={t('mkStudio.format')} aria-label={t('mkStudio.format')}>
                  {AD_FORMATS.map((f) => <option key={f.id} value={f.id}>{f.emoji} {t(`presets.fmt.${f.id}.label`)}</option>)}
                </select>
                {!replica && <select value={hookId} onChange={(e) => setHookId(e.target.value)} className={selCls} title={t('mkStudio.hook')} aria-label={t('mkStudio.hook')}>{AD_HOOKS.map((h) => <option key={h.id} value={h.id}>{h.id === 'none' ? t('mkStudio.hook') : t(`presets.hook.${h.id}`)}</option>)}</select>}
                {!replica && <select value={settingId} onChange={(e) => setSettingId(e.target.value)} className={selCls} title={t('mkStudio.setting')} aria-label={t('mkStudio.setting')}>{AD_SETTINGS.map((s) => <option key={s.id} value={s.id}>{s.id === 'none' ? t('mkStudio.setting') : t(`presets.setting.${s.id}`)}</option>)}</select>}
