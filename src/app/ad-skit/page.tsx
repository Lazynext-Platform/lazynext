      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,400px)_minmax(0,1fr)]">
            ...
            <div className="mt-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {uploadedImages.length < 4 && (
                  <label className="flex h-14 w-14 shrink-0 cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-line bg-surface hover:border-[#00b2fc]/60">
                    <input type="file" accept="image/*" multiple className="hidden" onChange={async (e) => {
                      const files = Array.from(e.target.files || []);
                      const urls = await Promise.all(files.map(imageToDataUrl));
                      setUploadedImages((a) => [...a, ...urls].slice(0, 4));
                    }} />
                    <UploadCloud className="h-5 w-5 text-fg-faint" />
                  </label>
                )}
                {uploadedImages.length < 4 && (
        ...
        <section className="space-y-5">
          {plan ? (
            <>
              <div className="rounded-2xl border border-line bg-popover p-5">
                <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold text-fg"><ImageIcon className="h-4 w-4 text-brand-accent" /> {t('adSkit.productPhotoTitle')}</h3>
              ...
              <div className="rounded-2xl border border-line bg-popover p-5">
                <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold text-fg"><Video className="h-4 w-4 text-brand-accent" /> {t('adSkit.finishedAd')}</h3>
                <div className="flex aspect-video items-center justify-center overflow-hidden rounded-xl border border-line bg-surface">
            ...
            <div className="flex min-h-[240px] flex-col items-center justify-center gap-2 rounded-2xl border border-line bg-popover p-8 text-center text-fg-placeholder">
