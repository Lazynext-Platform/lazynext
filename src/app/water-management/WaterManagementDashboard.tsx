...
import {
  Droplets, Gauge, ClipboardList, FlaskConical, Leaf, BarChart3,
  Search,
} from 'lucide-react';
import { Card, Badge, Button, EmptyState } from '@/components/ui';
import type {
  WaterMeter, WaterReading, WaterQualityTest, ConservationProgram,
  WaterManagementMetrics, WaterManagementStats,
} from '@/lib/services/water-management-service';

type TabId = 'overview' | 'meters' | 'readings' | 'quality' | 'conservation';
...
interface WaterManagementDashboardProps {
  ...
  metrics: WaterManagementMetrics;
  stats: WaterManagementStats;
}

const statusVariant = (status: string): 'success' | 'warning' | 'danger' | 'default' | 'info' => {
  if (['active', 'completed', 'verified'].includes(status)) return 'success';
  if (['pending', 'submitted', 'calibrating', 'planned', 'scheduled', 'in_progress'].includes(status)) return 'warning';
  if (['failed', 'decommissioned', 'offline', 'suspended', 'flagged', 'retest'].includes(status)) return 'danger';
  return 'info';
};

export function WaterManagementDashboard({
  meters, readings, qualityTests, programs, metrics, stats,
}: WaterManagementDashboardProps) {
  ...
  const filteredTests = useMemo(() => {
    if (!search) return qualityTests;
    const q = search.toLowerCase();
    return qualityTests.filter(
      (t) => t.type.toLowerCase().includes(q) || t.status.toLowerCase().includes(q) || t.parameter.toLowerCase().includes(q),
    );
  }, [qualityTests, search]);
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
            ...
            <Card className="p-4">
              <div className="text-xs text-fg-tertiary">Pending Tests</div>
              <div className="mt-1 text-2xl font-bold">{metrics.pendingTests}</div>
            </Card>
            <Card className="p-4">
          ...
          <Card className="p-6">
            <h3 className="mb-4 text-sm font-semibold">Test Status Breakdown</h3>
            <div className="flex flex-wrap gap-2">
              {Object.entries(stats.byTestStatus).map(([status, count]) => (
                <Badge key={status} variant={statusVariant(status)}>{status.replace('_', ' ')}: {count}</Badge>
              ))}
            </div>
          </Card>
      ...
      {tab === 'meters' && (
        <div className="space-y-3">
          {filteredMeters.length === 0 ? (
            <Card className="p-8"><EmptyState icon={Gauge} title="No meters" description="Water meters will appear here." /></Card>
          ) : (
      ...
      {tab === 'readings' && (
        <div className="space-y-3">
          {filteredReadings.length === 0 ? (
            <Card className="p-8"><EmptyState icon={ClipboardList} title="No readings" description="Water readings will appear here." /></Card>
          ) : (
      ...
      {tab === 'quality' && (
        <div className="space-y-3">
          {filteredTests.length === 0 ? (
            <Card className="p-8"><EmptyState icon={FlaskConical} title="No quality tests" description="Water quality tests will appear here." /></Card>
          ) : (
            filteredTests.map((t) => (
              <Card key={t.id} className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-fg-secondary">{t.parameter || 'No parameter'} · {t.passFail || 'No result'}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {t.value > 0 && <Badge variant="default">{t.value} {t.unit}</Badge>}
      ...
      {tab === 'conservation' && (
        <div className="space-y-3">
          {filteredPrograms.length === 0 ? (
            <Card className="p-8"><EmptyState icon={Leaf} title="No conservation programs" description="Conservation programs will appear here." /></Card>
          ) : (
