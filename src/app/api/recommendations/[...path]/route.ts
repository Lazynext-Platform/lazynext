...
const RESOURCES: Record<string, {
  ...
  'actions': {
    list: 'listRecommendationActions',
    create: 'createRecommendationAction',
    get: 'getRecommendationAction',
    update: 'updateRecommendationAction',
    delete: 'deleteRecommendationAction',
    actions: {
      'cancel': 'cancelRecommendationAction',
      'complete': 'completeRecommendationAction',
      'fail': 'failRecommendationAction',
      'start': 'startRecommendationAction',
    },
  },
  'feedback': {
    list: 'listRecommendationFeedbacks',
    create: 'createRecommendationFeedback',
    get: 'getRecommendationFeedback',
    update: 'updateRecommendationFeedback',
    delete: 'deleteRecommendationFeedback',
    actions: {
      'archive': 'archiveFeedback',
      'incorporate': 'incorporateFeedback',
      'record': 'recordFeedback',
      'review': 'reviewFeedback',
    },
  },
  'metrics': {
    list: 'getRecommendationEngineMetrics',
    actions: {},
  },
  'models': {
    list: 'listRecommendationModels',
    create: 'createRecommendationModel',
    get: 'getRecommendationModel',
    update: 'updateRecommendationModel',
    delete: 'deleteRecommendationModel',
    actions: {
      'activate': 'activateRecommendationModel',
      'archive': 'archiveRecommendationModel',
      'deprecate': 'deprecateRecommendationModel',
      'evaluate': 'evaluateRecommendationModel',
    },
  },
  'recommendations': {
  ...
  'stats': {
    list: 'getRecommendationEngineStats',
    actions: {},
  },
};
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const items = await (RecommendationService as any)[resource.list](ctx.ws.organizationId, opts);
    ...
    try {
      const item = await (RecommendationService as any)[resource.get](segments[1]);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const item = await (RecommendationService as any)[resource.create](ctx.ws.organizationId, ctx.ws.id, body, ctx.session.user.id);
      const key = segments[0].replace(/s$/, '');
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const item = await (RecommendationService as any)[action](segments[1], session.user.id);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[recommendations] action error:', e);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    const body = await req.json().catch(() => ({}));
    try {
      const item = await (RecommendationService as any)[resource.update](segments[1], body);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const ok = await (RecommendationService as any)[resource.delete](segments[1]);
