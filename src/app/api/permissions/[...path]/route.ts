...
const RESOURCES: Record<string, {
  ...
  'checks': {
    ...
    create: 'createPermissionCheck',
    get: 'getPermissionCheck',
    update: 'updatePermissionCheck',
    delete: 'deletePermissionCheck',
    actions: {
      'allow': 'allowCheck',
  ...
  'grants': {
    list: 'listPermissionGrants',
    create: 'createPermissionGrant',
    get: 'getPermissionGrant',
    update: 'updatePermissionGrant',
    delete: 'deletePermissionGrant',
    actions: {
      'expire': 'expirePermission',
  ...
  'metrics': {
    list: 'getPermissionMetrics',
    actions: {},
  },
  'policies': {
    list: 'listPermissionPolicies',
    create: 'createPermissionPolicy',
    get: 'getPermissionPolicy',
    update: 'updatePermissionPolicy',
    delete: 'deletePermissionPolicy',
    actions: {
      'activate': 'activatePermissionPolicy',
      'archive': 'archivePermissionPolicy',
      'deprecate': 'deprecatePermissionPolicy',
    },
  },
  'roles': {
    list: 'listPermissionRoles',
    create: 'createPermissionRole',
    get: 'getPermissionRole',
    update: 'updatePermissionRole',
    delete: 'deletePermissionRole',
    actions: {
      'activate': 'activatePermissionRole',
      'deactivate': 'deactivatePermissionRole',
      'deprecate': 'deprecatePermissionRole',
    },
  },
  'stats': {
    list: 'getPermissionStats',
    actions: {},
  },
};
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const items = await (PermissionService as any)[resource.list](ctx.ws.organizationId, opts);
    ...
    try {
      const item = await (PermissionService as any)[resource.get](segments[1]);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const item = await (PermissionService as any)[resource.create](ctx.ws.organizationId, ctx.ws.id, body, ctx.session.user.id);
      const key = segments[0].replace(/s$/, '');
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const item = await (PermissionService as any)[action](segments[1], session.user.id);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[permissions] action error:', e);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    const body = await req.json().catch(() => ({}));
    try {
      const item = await (PermissionService as any)[resource.update](segments[1], body);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const ok = await (PermissionService as any)[resource.delete](segments[1]);
