...
const RESOURCES: Record<string, {
  ...
  'actions': {
    list: 'listOpportunityActions',
    create: 'createOpportunityAction',
    get: 'getOpportunityAction',
    update: 'updateOpportunityAction',
    delete: 'deleteOpportunityAction',
    actions: {
      'cancel': 'cancelOpportunityAction',
      'complete': 'completeOpportunityAction',
      'fail': 'failOpportunityAction',
      'start': 'startOpportunityAction',
    },
  },
  'metrics': {
    list: 'getOpportunityDetectionMetrics',
    actions: {},
  },
  'opportunities': {
    ...
    actions: {
      'approve': 'approveOpportunity',
      'archive': 'archiveOpportunity',
      'evaluate': 'evaluateOpportunity',
      'pursue': 'pursueOpportunity',
      'realize': 'realizeOpportunity',
      'reject': 'rejectOpportunity',
    },
  },
  'rules': {
    list: 'listDetectionRules',
    create: 'createDetectionRule',
    get: 'getDetectionRule',
    update: 'updateDetectionRule',
    delete: 'deleteDetectionRule',
    actions: {
      'activate': 'activateDetectionRule',
      'archive': 'archiveDetectionRule',
      'deprecate': 'deprecateDetectionRule',
      'pause': 'pauseDetectionRule',
    },
  },
  'scores': {
    list: 'listOpportunityScores',
    create: 'createOpportunityScore',
    get: 'getOpportunityScore',
    update: 'updateOpportunityScore',
    delete: 'deleteOpportunityScore',
    actions: {
      'adjust': 'adjustScore',
  ...
  'stats': {
    list: 'getOpportunityDetectionStats',
    actions: {},
  },
};
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const items = await (OpportunityService as any)[resource.list](ctx.ws.organizationId, opts);
    ...
    try {
      const item = await (OpportunityService as any)[resource.get](segments[1]);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const item = await (OpportunityService as any)[resource.create](ctx.ws.organizationId, ctx.ws.id, body, ctx.session.user.id);
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item }, { status: 201 });
    } catch (e) {
      console.error('[opportunities] create error:', e);
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const item = await (OpportunityService as any)[action](segments[1], session.user.id);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[opportunities] action error:', e);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    const body = await req.json().catch(() => ({}));
    try {
      const item = await (OpportunityService as any)[resource.update](segments[1], body);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[opportunities] update error:', e);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const ok = await (OpportunityService as any)[resource.delete](segments[1]);
      if (!ok) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      return NextResponse.json({ ok: true });
    } catch (e) {
      console.error('[opportunities] delete error:', e);
