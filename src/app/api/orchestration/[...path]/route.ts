...
const RESOURCES: Record<string, {
  ...
  'collaborations': {
    ...
    get: 'getCollaboration',
    actions: {
      'end': 'endCollaboration',
    },
  },
  'delegate': {
    create: 'delegateTask',
  ...
  'handoff': {
    create: 'coordinateHandoff',
    actions: {},
  },
  'resolve-conflict': {
    create: 'resolveConflict',
    actions: {},
  },
  'share-context': {
    create: 'shareContext',
  ...
  'suggest': {
    create: 'suggestCollaboration',
    actions: {},
  },
  'workload': {
    list: 'getAgentWorkload',
    actions: {},
  },
};
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const items = await (OrchestrationService as any)[resource.list](ctx.ws.organizationId, opts);
    ...
    try {
      const item = await (OrchestrationService as any)[resource.get](segments[1]);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const item = await (OrchestrationService as any)[resource.create](ctx.ws.organizationId, ctx.ws.id, body, ctx.session.user.id);
      const key = segments[0].replace(/s$/, '');
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const item = await (OrchestrationService as any)[action](segments[1], session.user.id);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[orchestration] action error:', e);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    const body = await req.json().catch(() => ({}));
    try {
      const item = await (OrchestrationService as any)[resource.update](segments[1], body);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[orchestration] update error:', e);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const ok = await (OrchestrationService as any)[resource.delete](segments[1]);
