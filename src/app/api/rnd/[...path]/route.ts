...
const RESOURCES: Record<string, {
  ...
  'innovations': {
    ...
    create: 'createInnovation',
    get: 'getInnovation',
    update: 'updateInnovation',
    delete: 'deleteInnovation',
    actions: {
      'approve': 'approveInnovation',
      'develop': 'developInnovation',
      'evaluate': 'evaluateInnovation',
      'launch': 'launchInnovation',
    },
  },
  'metrics': {
    list: 'getRndMetrics',
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const items = await (RndService as any)[resource.list](ctx.ws.organizationId, opts);
      return NextResponse.json({ [segments[0]]: items });
    } catch (e) {
      console.error('[rnd] list error:', e);
    ...
    try {
      const item = await (RndService as any)[resource.get](segments[1]);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[rnd] get error:', e);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const item = await (RndService as any)[resource.create](ctx.ws.organizationId, ctx.ws.id, body, ctx.session.user.id);
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item }, { status: 201 });
    } catch (e) {
      console.error('[rnd] create error:', e);
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const item = await (RndService as any)[action](segments[1], session.user.id);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[rnd] action error:', e);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    const body = await req.json().catch(() => ({}));
    try {
      const item = await (RndService as any)[resource.update](segments[1], body);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[rnd] update error:', e);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const ok = await (RndService as any)[resource.delete](segments[1]);
      if (!ok) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      return NextResponse.json({ ok: true });
    } catch (e) {
      console.error('[rnd] delete error:', e);
