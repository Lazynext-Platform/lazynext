...
const RESOURCES: Record<string, {
  ...
  'cost-summary': {
    list: 'getTravelCostSummary',
    actions: {},
  },
  'requests': {
    list: 'listTravelRequests',
    create: 'createTravelRequest',
    get: 'getTravelRequest',
    update: 'updateTravelRequest',
    actions: {
      'approve': 'approveTravelRequest',
      'bookings': 'addBooking',
      'cancel': 'cancelTravelRequest',
      'convert-expense': 'convertToExpense',
      'itinerary': 'addItineraryItem',
      'reject': 'rejectTravelRequest',
    },
  },
  'stats': {
    list: 'getStats',
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const items = await (TravelService as any)[resource.list](session.user.id);
      return NextResponse.json({ [segments[0]]: items });
    } catch (e) {
      console.error('[travel] list error:', e);
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const item = await (TravelService as any)[resource.get](segments[1]);
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 1) {
    ...
    try {
      const item = await (TravelService as any)[resource.create](body);
      const key = segments[0].replace(/s$/, '');
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const item = await (TravelService as any)[action](segments[1], session.user.id);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
      console.error('[travel] action error:', e);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    const body = await req.json().catch(() => ({}));
    try {
      const item = await (TravelService as any)[resource.update](segments[1], body);
      if (!item) return NextResponse.json({ error: 'not_found' }, { status: 404 });
      const key = segments[0].replace(/s$/, '');
      return NextResponse.json({ [key]: item });
    } catch (e) {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (segments.length === 2) {
    ...
    if (!session?.user?.id) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
    try {
      const ok = await (TravelService as any)[resource.delete](segments[1]);
