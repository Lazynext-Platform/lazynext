...
/**
 * GET /api/creative/creative-ad-surprise-element-designer
 * Returns the credit cost, schema info, and supported platforms/surprise
 * types (no auth required for catalog metadata — same pattern as other
 * creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-surprise-element-designer',
    ...
    schema: {
      ...
      surpriseTypes: VALID_SURPRISE_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdSurpriseElementDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_SURPRISE_ELEMENT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-surprise-element-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateSurpriseElements(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-surprise-element-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-surprise-element-designer', 'generate_failed');
