...
/**
 * GET /api/creative/ad-creative-callback-memory-designer
 * Returns the credit cost, schema info, and supported platforms/callback
 * types/reward types (no auth required for catalog metadata — same
 * pattern as other creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-callback-memory-designer',
    ...
    schema: {
      ...
      callbackTypes: VALID_CALLBACK_TYPES,
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeCallbackMemoryDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_CALLBACK_MEMORY_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-callback-memory-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateCallbacks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-callback-memory-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-callback-memory-designer', 'generate_failed');
