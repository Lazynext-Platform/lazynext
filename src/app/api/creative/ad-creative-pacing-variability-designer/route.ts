...
/**
 * GET /api/creative/ad-creative-pacing-variability-designer
 * Returns the credit cost, schema info, and supported platforms/speed
 * levels/impacts/directions (no auth required for catalog metadata — same
 * pattern as other creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-pacing-variability-designer',
    ...
    schema: {
      ...
      speedLevels: VALID_SPEED_LEVELS,
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativePacingVariabilityDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_PACING_VARIABILITY_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-pacing-variability-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generatePacingVariability(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-pacing-variability-designer').catch(
      () => {},
    );
    const safe = safeError(
      e,
      'creative/ad-creative-pacing-variability-designer',
      'generate_failed',
    );
