...
/**
 * GET /api/creative/ad-creative-risk-reversal-designer
 * Returns the credit cost, schema info, and supported platforms/reversal types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-risk-reversal-designer',
    ...
    schema: {
      ...
      reversalTypes: VALID_REVERSAL_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeRiskReversalDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_RISK_REVERSAL_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-risk-reversal-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateRiskReversals(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-risk-reversal-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-risk-reversal-designer', 'generate_failed');
