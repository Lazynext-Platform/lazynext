...
/**
 * GET /api/creative/creative-ad-loss-aversion-framing-designer
 * Returns the credit cost, schema info, and supported platforms/loss types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-loss-aversion-framing-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdLossAversionFramingDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_LOSS_AVERSION_FRAMING_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-loss-aversion-framing-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateLossAversionFrameworks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-loss-aversion-framing-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-loss-aversion-framing-designer', 'generate_failed');
