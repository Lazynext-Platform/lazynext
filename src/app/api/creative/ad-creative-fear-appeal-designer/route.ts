...
/**
 * GET /api/creative/ad-creative-fear-appeal-designer
 * Returns the credit cost, schema info, and supported platforms/fear types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-fear-appeal-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeFearAppealDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_FEAR_APPEAL_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-fear-appeal-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateFearAppeals(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-fear-appeal-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-fear-appeal-designer', 'generate_failed');
