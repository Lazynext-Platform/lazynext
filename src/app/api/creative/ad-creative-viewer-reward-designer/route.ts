...
/**
 * GET /api/creative/ad-creative-viewer-reward-designer
 * Returns the credit cost, schema info, and supported platforms/reward types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-viewer-reward-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeViewerRewardDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_VIEWER_REWARD_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-viewer-reward-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateViewerRewards(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-viewer-reward-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-viewer-reward-designer', 'generate_failed');
