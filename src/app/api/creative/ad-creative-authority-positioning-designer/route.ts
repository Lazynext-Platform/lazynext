...
/**
 * GET /api/creative/ad-creative-authority-positioning-designer
 * Returns the credit cost, schema info, and supported platforms/authority types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-authority-positioning-designer',
    ...
    schema: {
      ...
      authorityTypes: VALID_AUTHORITY_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeAuthorityPositioningDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_AUTHORITY_POSITIONING_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-authority-positioning-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateAuthorityPositionings(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-authority-positioning-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-authority-positioning-designer', 'generate_failed');
