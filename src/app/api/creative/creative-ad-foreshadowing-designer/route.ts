...
/**
 * GET /api/creative/creative-ad-foreshadowing-designer
 * Returns the credit cost, schema info, and supported platforms/hint types/
 * viewer discovery values (no auth required for catalog metadata — same
 * pattern as other creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-foreshadowing-designer',
    ...
    schema: {
      ...
      hintTypes: VALID_HINT_TYPES,
      viewerDiscovery: VALID_VIEWER_DISCOVERY,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdForeshadowingDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_FORESHADOWING_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-foreshadowing-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateForeshadowing(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-foreshadowing-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-foreshadowing-designer', 'generate_failed');
