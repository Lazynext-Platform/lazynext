...
/**
 * GET /api/creative/ad-creative-demonstration-framework-designer
 * Returns the credit cost, schema info, and supported platforms/demo types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-demonstration-framework-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeDemonstrationFrameworkDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_DEMONSTRATION_FRAMEWORK_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-demonstration-framework-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateDemonstrationFrameworks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-demonstration-framework-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-demonstration-framework-designer', 'generate_failed');
