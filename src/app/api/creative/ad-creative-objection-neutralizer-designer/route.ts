...
/**
 * GET /api/creative/ad-creative-objection-neutralizer-designer
 * Returns the credit cost, schema info, and supported platforms/objection types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-objection-neutralizer-designer',
    ...
    schema: {
      ...
      objectionTypes: VALID_OBJECTION_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeObjectionNeutralizerDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_OBJECTION_NEUTRALIZER_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-objection-neutralizer-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateObjectionNeutralizers(input, planTier);
