...
/**
 * GET /api/creative/ad-creative-humor-appeal-designer
 * Returns the credit cost, schema info, and supported platforms/humor types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-humor-appeal-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeHumorAppealDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_HUMOR_APPEAL_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-humor-appeal-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateHumorAppeals(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-humor-appeal-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-humor-appeal-designer', 'generate_failed');
