...
/**
 * GET /api/creative/ad-creative-curiosity-loop-designer
 * Returns the credit cost, schema info, and supported platforms/loop types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-curiosity-loop-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeCuriosityLoopDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_CURIOSITY_LOOP_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-curiosity-loop-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateCuriosityLoops(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-curiosity-loop-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-curiosity-loop-designer', 'generate_failed');
