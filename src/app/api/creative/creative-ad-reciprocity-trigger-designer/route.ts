...
/**
 * GET /api/creative/creative-ad-reciprocity-trigger-designer
 * Returns the credit cost, schema info, and supported platforms/reciprocity types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-reciprocity-trigger-designer',
    ...
    schema: {
      ...
      reciprocityTypes: VALID_RECIPROCITY_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdReciprocityTriggerDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_RECIPROCITY_TRIGGER_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-reciprocity-trigger-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateReciprocityFrameworks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-reciprocity-trigger-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-reciprocity-trigger-designer', 'generate_failed');
