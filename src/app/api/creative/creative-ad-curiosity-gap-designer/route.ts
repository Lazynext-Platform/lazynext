...
/**
 * GET /api/creative/creative-ad-curiosity-gap-designer
 * Returns the credit cost, schema info, and supported platforms/gap types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-curiosity-gap-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdCuriosityGapDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_CURIOSITY_GAP_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-curiosity-gap-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateCuriosityGaps(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-curiosity-gap-designer').catch(() => {});
    const safe = safeError(
      e,
      'creative/creative-ad-curiosity-gap-designer',
      'generate_failed',
    );
