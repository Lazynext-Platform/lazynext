...
/**
 * GET /api/creative/ad-creative-rhythm-pacing-optimizer
 * Returns the credit cost, schema info, and supported platforms/tempos/
 * impacts (no auth required for catalog metadata — same pattern as other
 * creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-rhythm-pacing-optimizer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeRhythmPacingOptimizerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_RHYTHM_PACING_OPTIMIZER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-rhythm-pacing-optimizer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateRhythmOptimization(input, planTier);
