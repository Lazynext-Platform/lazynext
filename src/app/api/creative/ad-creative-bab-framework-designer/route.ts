...
/**
 * GET /api/creative/ad-creative-bab-framework-designer
 * Returns the credit cost, schema info, and supported platforms/transformation types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-bab-framework-designer',
    ...
    schema: {
      ...
      transformationTypes: VALID_TRANSFORMATION_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeBABFrameworkDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_BAB_FRAMEWORK_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-bab-framework-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateBABFrameworks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-bab-framework-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-bab-framework-designer', 'generate_failed');
