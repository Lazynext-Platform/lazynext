...
/**
 * GET /api/creative/creative-ad-offer-architecture-designer
 * Returns the credit cost, schema info, and supported platforms/offer component types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-offer-architecture-designer',
    ...
    schema: {
      ...
      offerComponentTypes: VALID_OFFER_COMPONENT_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdOfferArchitectureDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_OFFER_ARCHITECTURE_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-offer-architecture-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateOfferArchitectures(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-offer-architecture-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-offer-architecture-designer', 'generate_failed');
