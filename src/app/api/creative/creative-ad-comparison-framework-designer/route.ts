...
/**
 * GET /api/creative/creative-ad-comparison-framework-designer
 * Returns the credit cost, schema info, and supported platforms/comparison
 * types (no auth required for catalog metadata — same pattern as other
 * creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-comparison-framework-designer',
    ...
    schema: {
      ...
      comparisonTypes: VALID_COMPARISON_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdComparisonFrameworkDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_COMPARISON_FRAMEWORK_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-comparison-framework-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateComparisonFrameworks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-comparison-framework-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-comparison-framework-designer', 'generate_failed');
