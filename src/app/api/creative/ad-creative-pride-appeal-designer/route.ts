...
/**
 * GET /api/creative/ad-creative-pride-appeal-designer
 * Returns the credit cost, schema info, and supported platforms/pride types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-pride-appeal-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativePrideAppealDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_PRIDE_APPEAL_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-pride-appeal-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generatePrideAppeals(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-pride-appeal-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-pride-appeal-designer', 'generate_failed');
