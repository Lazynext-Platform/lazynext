...
/**
 * GET /api/creative/creative-ad-liking-affinity-designer
 * Returns the credit cost, schema info, and supported platforms/affinity types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-liking-affinity-designer',
    ...
    schema: {
      ...
      affinityTypes: VALID_AFFINITY_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdLikingAffinityDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_LIKING_AFFINITY_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-liking-affinity-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateLikingAffinities(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-liking-affinity-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-liking-affinity-designer', 'generate_failed');
