...
/**
 * GET /api/creative/creative-ad-value-ladder-designer
 * Returns the credit cost, schema info, and supported platforms/step types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-value-ladder-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdValueLadderDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_VALUE_LADDER_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-value-ladder-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateValueLadders(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-value-ladder-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-value-ladder-designer', 'generate_failed');
