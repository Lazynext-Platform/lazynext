...
/**
 * GET /api/creative/ad-creative-decoy-effect-designer
 * Returns the credit cost, schema info, and supported platforms/decoy types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-decoy-effect-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeDecoyEffectDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_DECOY_EFFECT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-decoy-effect-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateDecoyEffects(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-decoy-effect-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-decoy-effect-designer', 'generate_failed');
