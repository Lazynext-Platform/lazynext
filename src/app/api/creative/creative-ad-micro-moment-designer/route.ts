...
/**
 * GET /api/creative/creative-ad-micro-moment-designer
 * Returns the credit cost, schema info, and supported platforms/moment types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-micro-moment-designer',
    ...
    schema: {
      ...
      momentTypes: VALID_MOMENT_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdMicroMomentDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_MICRO_MOMENT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-micro-moment-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateMicroMoments(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-micro-moment-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-micro-moment-designer', 'generate_failed');
