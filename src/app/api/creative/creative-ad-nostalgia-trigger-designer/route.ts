...
/**
 * GET /api/creative/creative-ad-nostalgia-trigger-designer
 * Returns the credit cost, schema info, and supported platforms/nostalgia
 * types (no auth required for catalog metadata — same pattern as other
 * creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-nostalgia-trigger-designer',
    ...
    schema: {
      ...
      nostalgiaTypes: VALID_NOSTALGIA_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdNostalgiaTriggerDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_NOSTALGIA_TRIGGER_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-nostalgia-trigger-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateNostalgiaTriggers(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-nostalgia-trigger-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-nostalgia-trigger-designer', 'generate_failed');
