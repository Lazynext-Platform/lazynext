...
/**
 * GET /api/creative/creative-ad-endowment-effect-designer
 * Returns the credit cost, schema info, and supported platforms/endowment types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-endowment-effect-designer',
    ...
    schema: {
      ...
      endowmentTypes: VALID_ENDOWMENT_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdEndowmentEffectDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_ENDOWMENT_EFFECT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-endowment-effect-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateEndowmentEffects(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-endowment-effect-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-endowment-effect-designer', 'generate_failed');
