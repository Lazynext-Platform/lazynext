...
/**
 * GET /api/creative/ad-creative-scarcity-frame-designer
 * Returns the credit cost, schema info, and supported platforms/frame types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-scarcity-frame-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeScarcityFrameDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_SCARCITY_FRAME_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-scarcity-frame-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateScarcityFrames(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-scarcity-frame-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-scarcity-frame-designer', 'generate_failed');
