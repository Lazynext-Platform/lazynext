...
/**
 * GET /api/creative/creative-ad-tension-release-strategist
 * Returns the credit cost, schema info, and supported platforms/relief levels
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-tension-release-strategist',
    ...
    schema: {
      ...
      reliefLevels: VALID_RELIEF_LEVELS,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdTensionReleaseStrategistInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_TENSION_RELEASE_STRATEGIST_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-tension-release-strategist');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateTensionStrategy(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-tension-release-strategist').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-tension-release-strategist', 'generate_failed');
