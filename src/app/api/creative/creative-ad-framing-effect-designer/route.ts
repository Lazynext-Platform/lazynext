...
/**
 * GET /api/creative/creative-ad-framing-effect-designer
 * Returns the credit cost, schema info, and supported platforms/framing types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-framing-effect-designer',
    ...
    schema: {
      ...
      framingTypes: VALID_FRAMING_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdFramingEffectDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_FRAMING_EFFECT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-framing-effect-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateFramingEffects(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-framing-effect-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-framing-effect-designer', 'generate_failed');
