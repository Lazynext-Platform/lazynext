...
/**
 * GET /api/creative/creative-messaging-framework-builder
...
export async function GET() {
  return NextResponse.json({
    feature: 'creative-messaging-framework-builder',
    creditCost: CREATIVE_MESSAGING_FRAMEWORK_BUILDER_CREDIT_COST,
    schema: {
      input: {
        productOrBrand: `string (required, max ${MAX_PRODUCT_LENGTH} chars)`,
        valueProposition: `string (required, max ${MAX_VALUE_PROP_LENGTH} chars)`,
...
async function __byokPOST(req: Request) {
  ...
  const valueProposition =
    typeof body.valueProposition === 'string' ? body.valueProposition.trim().slice(0, MAX_VALUE_PROP_LENGTH) : '';
  ...
  const input: CreativeMessagingFrameworkBuilderInput = {
    productOrBrand,
    valueProposition,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_MESSAGING_FRAMEWORK_BUILDER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-messaging-framework-builder');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateMessagingFramework(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-messaging-framework-builder').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-messaging-framework-builder', 'generate_failed');
