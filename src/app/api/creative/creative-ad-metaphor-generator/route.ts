...
/**
 * GET /api/creative/creative-ad-metaphor-generator
...
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-metaphor-generator',
    creditCost: CREATIVE_AD_METAPHOR_GENERATOR_CREDIT_COST,
    schema: {
      input: {
        productOrBrand: `string (required, max ${MAX_PRODUCT_LENGTH} chars)`,
        benefit: `string (required, max ${MAX_BENEFIT_LENGTH} chars)`,
...
async function __byokPOST(req: Request) {
  ...
  const benefit =
    typeof body.benefit === 'string' ? body.benefit.trim().slice(0, MAX_BENEFIT_LENGTH) : '';
  ...
  const input: CreativeAdMetaphorGeneratorInput = {
    productOrBrand,
    benefit,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_METAPHOR_GENERATOR_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-metaphor-generator');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateMetaphors(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-metaphor-generator').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-metaphor-generator', 'generate_failed');
