...
/**
 * GET /api/creative/creative-ad-price-framing-designer
 * Returns the credit cost, schema info, and supported platforms/framing
 * types (no auth required for catalog metadata — same pattern as other
 * creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-price-framing-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdPriceFramingDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_PRICE_FRAMING_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-price-framing-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generatePriceFramings(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-price-framing-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-price-framing-designer', 'generate_failed');
