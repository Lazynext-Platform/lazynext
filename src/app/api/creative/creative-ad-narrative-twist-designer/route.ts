...
/**
 * GET /api/creative/creative-ad-narrative-twist-designer
 * Returns the credit cost, schema info, and supported platforms/twist types/
 * emotional impacts (no auth required for catalog metadata — same pattern as
 * other creative catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-narrative-twist-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdNarrativeTwistDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_NARRATIVE_TWIST_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-narrative-twist-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateTwists(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-narrative-twist-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-narrative-twist-designer', 'generate_failed');
