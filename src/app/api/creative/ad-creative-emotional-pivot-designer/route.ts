...
/**
 * GET /api/creative/ad-creative-emotional-pivot-designer
 * Returns the credit cost, schema info, and supported platforms/pivot types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-emotional-pivot-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeEmotionalPivotDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_EMOTIONAL_PIVOT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-emotional-pivot-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateEmotionalPivots(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-emotional-pivot-designer').catch(
      () => {},
    );
    const safe = safeError(
      e,
      'creative/ad-creative-emotional-pivot-designer',
      'generate_failed',
    );
