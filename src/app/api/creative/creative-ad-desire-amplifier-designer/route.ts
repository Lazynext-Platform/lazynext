...
/**
 * GET /api/creative/creative-ad-desire-amplifier-designer
 * Returns the credit cost, schema info, and supported platforms/amplifier types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-desire-amplifier-designer',
    ...
    schema: {
      ...
      amplifierTypes: VALID_AMPLIFIER_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdDesireAmplifierDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_DESIRE_AMPLIFIER_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-desire-amplifier-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateDesireAmplifiers(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-desire-amplifier-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-desire-amplifier-designer', 'generate_failed');
