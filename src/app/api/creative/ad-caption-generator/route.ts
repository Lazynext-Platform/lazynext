...
/**
 * GET /api/creative/ad-caption-generator
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCaptionGeneratorInput = {
    productOrBrand,
    platform,
    tone,
    count,
  ...
  const cost = AD_CAPTION_GENERATOR_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-caption-generator');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateAdCaptions(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-caption-generator').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-caption-generator', 'generate_failed');
