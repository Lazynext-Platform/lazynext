...
/**
 * GET /api/creative/ad-creative-social-momentum-designer
 * Returns the credit cost, schema info, and supported platforms/momentum types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-social-momentum-designer',
    ...
    schema: {
      ...
      momentumTypes: VALID_MOMENTUM_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeSocialMomentumDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_SOCIAL_MOMENTUM_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-social-momentum-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateSocialMomentum(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-social-momentum-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-social-momentum-designer', 'generate_failed');
