...
/**
 * GET /api/creative/ad-creative-tension-release-designer
 * Returns the credit cost, schema info, and supported platforms/cycle types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-tension-release-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeTensionReleaseDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_TENSION_RELEASE_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-tension-release-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateTensionRelease(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-tension-release-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-tension-release-designer', 'generate_failed');
