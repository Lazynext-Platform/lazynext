...
/**
 * GET /api/creative/ad-creative-belief-shift-designer
 * Returns the credit cost, schema info, and supported platforms/shift types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-belief-shift-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeBeliefShiftDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_BELIEF_SHIFT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-belief-shift-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateBeliefShifts(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-belief-shift-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-belief-shift-designer', 'generate_failed');
