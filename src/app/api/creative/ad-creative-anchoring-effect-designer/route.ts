...
/**
 * GET /api/creative/ad-creative-anchoring-effect-designer
 * Returns the credit cost, schema info, and supported platforms/anchor types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-anchoring-effect-designer',
    ...
    schema: {
      ...
      anchorTypes: VALID_ANCHOR_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeAnchoringEffectDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_ANCHORING_EFFECT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-anchoring-effect-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateAnchoringFrameworks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-anchoring-effect-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-anchoring-effect-designer', 'generate_failed');
