...
/**
 * GET /api/creative/ad-creative-emotional-anchor-designer
...
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-emotional-anchor-designer',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeEmotionalAnchorDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_EMOTIONAL_ANCHOR_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-emotional-anchor-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateEmotionalAnchors(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-emotional-anchor-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-emotional-anchor-designer', 'generate_failed');
