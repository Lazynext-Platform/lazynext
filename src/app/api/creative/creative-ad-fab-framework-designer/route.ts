...
/**
 * GET /api/creative/creative-ad-fab-framework-designer
 * Returns the credit cost, schema info, and supported platforms/benefit types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-fab-framework-designer',
    ...
    schema: {
      ...
      benefitTypes: VALID_BENEFIT_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdFABFrameworkDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_FAB_FRAMEWORK_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-fab-framework-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateFABFrameworks(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-fab-framework-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-fab-framework-designer', 'generate_failed');
