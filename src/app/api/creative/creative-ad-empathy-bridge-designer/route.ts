...
/**
 * GET /api/creative/creative-ad-empathy-bridge-designer
 * Returns the credit cost, schema info, and supported platforms/bridge types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-empathy-bridge-designer',
    ...
    schema: {
      ...
      bridgeTypes: VALID_BRIDGE_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdEmpathyBridgeDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_EMPATHY_BRIDGE_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-empathy-bridge-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateEmpathyBridges(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-empathy-bridge-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-empathy-bridge-designer', 'generate_failed');
