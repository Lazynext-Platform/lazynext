...
/**
 * GET /api/creative/creative-ad-micro-commitment-designer
 * Returns the credit cost, schema info, and supported platforms/commitment types
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-micro-commitment-designer',
    ...
    schema: {
      ...
      commitmentTypes: VALID_COMMITMENT_TYPES,
    },
  });
}
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdMicroCommitmentDesignerInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_MICRO_COMMITMENT_DESIGNER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-micro-commitment-designer');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateMicroCommitments(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-micro-commitment-designer').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-micro-commitment-designer', 'generate_failed');
