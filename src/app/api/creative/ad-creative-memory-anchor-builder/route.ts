...
/**
 * GET /api/creative/ad-creative-memory-anchor-builder
...
export async function GET() {
  return NextResponse.json({
    feature: 'ad-creative-memory-anchor-builder',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdCreativeMemoryAnchorBuilderInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = AD_CREATIVE_MEMORY_ANCHOR_BUILDER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-creative-memory-anchor-builder');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateMemoryAnchors(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-creative-memory-anchor-builder').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-creative-memory-anchor-builder', 'generate_failed');
