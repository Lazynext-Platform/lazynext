...
/**
 * GET /api/creative/creative-ad-anticipation-builder
 * Returns the credit cost, schema info, and supported platforms/intensities
 * (no auth required for catalog metadata — same pattern as other creative
 * catalog endpoints).
 */
export async function GET() {
  return NextResponse.json({
    feature: 'creative-ad-anticipation-builder',
...
async function __byokPOST(req: Request) {
  ...
  const input: CreativeAdAnticipationBuilderInput = {
    productOrBrand,
    content,
    targetAudience,
    platform,
  ...
  const cost = CREATIVE_AD_ANTICIPATION_BUILDER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:creative-ad-anticipation-builder');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generateAnticipation(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:creative-ad-anticipation-builder').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/creative-ad-anticipation-builder', 'generate_failed');
