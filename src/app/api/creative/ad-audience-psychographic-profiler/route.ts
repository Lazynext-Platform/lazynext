...
/**
 * GET /api/creative/ad-audience-psychographic-profiler
...
export async function GET() {
  return NextResponse.json({
    feature: 'ad-audience-psychographic-profiler',
...
async function __byokPOST(req: Request) {
  ...
  const input: AdAudiencePsychographicProfilerInput = {
    productOrBrand,
    targetAudience,
    platform,
    dryRun,
  ...
  const cost = AD_AUDIENCE_PSYCHOGRAPHIC_PROFILER_CREDIT_COST;

  try {
    await deductCredits(uid, cost, 'creative:ad-audience-psychographic-profiler');
  } catch (e) {
    return NextResponse.json(
      {
        error:
  ...
  try {
    const result = await generatePsychographicProfile(input, planTier);
    return NextResponse.json({ result });
  } catch (e) {
    await refundCredits(uid, cost, 'creative:ad-audience-psychographic-profiler').catch(() => {});
    const { error, status } = safeAtlasError(e, 'creative/ad-audience-psychographic-profiler', 'generate_failed');
