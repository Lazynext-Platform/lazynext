...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const permit = await EnvironmentalComplianceService.getPermit(id);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const permit = await EnvironmentalComplianceService.updatePermit(id, body);
    if (!permit) return NextResponse.json({ error: 'not_found' }, { status: 404 });
    return NextResponse.json({ permit });
  } catch (e) {
    console.error('[environmental-compliance/permits] update error:', e);
