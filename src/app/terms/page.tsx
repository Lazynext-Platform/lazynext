...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const legal = (messages[locale] as any)?.legal?.terms || (messages.en as any).legal.terms;
  return {
...
export default async function TermsPage() {
  const localeCookie = (await cookies()).get('locale')?.value;
  const locale = ((LOCALES as readonly string[]).includes(localeCookie || '') ? localeCookie : 'en') as Locale;
  const t = (messages[locale] as any)?.legal?.terms || (messages.en as any).legal.terms;
    ...
    <main className="min-h-screen text-[#f7f7f8]" style={{ backgroundColor: '#131416', colorScheme: 'dark' }}>
      <div className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="mt-10 space-y-8 text-sm leading-relaxed text-white/70">
          ...
          <section>
            <h2 className="text-lg font-semibold text-white/90">{t.s8Title}</h2>
            <p className="mt-3">{t.s8Body}</p>
          </section>

          <section>
            <h2 className="text-lg font-semibold text-white/90">{t.s9Title}</h2>
            <p className="mt-3">{t.s9Body}</p>
          </section>

          <section>
            <h2 className="text-lg font-semibold text-white/90">{t.s10Title}</h2>
