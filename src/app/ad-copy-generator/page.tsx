      ...
      <div className="mx-auto max-w-5xl px-4 py-8 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input id="acgKeywords" type="text" value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder={t('common.phKeywords')} className="w-full rounded-lg border border-border bg-bg-card px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent" disabled={loading} />
              </div>
            </div>
          </div>
          ...
          <button onClick={generate} disabled={loading || !source.trim()} className="rounded-lg bg-brand-accent px-4 py-2 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50 flex items-center gap-2">
            ...
            {loading ? t('adCopyGenerator.generating') : `${t('adCopyGenerator.generate')} (${CREDIT_COST} ${t('common.creditsLower')})`}
          </button>
        </div>
