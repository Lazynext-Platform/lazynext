    ...
    <div className="min-h-screen bg-app">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 pb-24">
        <div className="pt-6 pb-8">
          ...
          {generating > 0 && (
            <div className="mt-3 inline-flex items-center gap-2 rounded-full border border-[#00b2fc]/30 bg-[#00b2fc]/[0.08] px-3 py-1 text-xs text-fg-secondary">
              <Loader2 className="h-3.5 w-3.5 animate-spin" style={{ color: 'var(--color-brand-accent)' }} />
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {items.map((c) => {
              ...
              if (c.assets && c.assets.kind === 'drama') {
                ...
                return (
                  <button key={c.id} onClick={() => router.push(`/my-work/${c.id}`)} className="group overflow-hidden rounded-2xl border border-line bg-black/30 text-left">
                    <div className="relative aspect-[9/16] w-full">
                      ...
                      <span className="absolute left-2 top-2 rounded bg-black/60 px-2 py-0.5 text-[10px] font-medium text-fg">{t('myWork.drama')}</span>
                      <span className="absolute right-2 top-2 inline-flex items-center gap-1 rounded bg-black/60 px-2 py-0.5 text-[10px] font-medium text-fg"><Film className="h-3 w-3" />{t('myWork.folder')}</span>
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent p-2">
                        <div className="text-[11px] font-medium text-fg">{stateLabel}</div>
                      </div>
                  ...
                  <div key={c.id} className="overflow-hidden rounded-2xl border border-[#00b2fc]/30 bg-black/30">
                    <div className="relative aspect-[9/16] w-full">
                      ...
                      <div className="absolute inset-0 grid place-items-center gap-2 bg-black/50">
                        ...
                        <span className="text-xs font-medium text-fg">{t('myWork.generatingDots')}</span>
                      </div>
                      {badge && <span className="absolute left-2 top-2 rounded bg-black/60 px-2 py-0.5 text-[10px] font-medium text-fg">{badge}</span>}
                    </div>
                    <div className="p-3">
