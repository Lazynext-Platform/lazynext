...
MCP-style creative tool contracts with JSON schemas, validation, and execution. This capability
...
Implement `tools` as a creative library in `src/lib/creative/tools.ts`
that:
...
- Exposes API routes under `/api/creative/tools`

### API

- `GET /api/creative/tools` — returns credit cost and schema
- `POST /api/creative/tools` — executes the creative tools
...
- Covered by unit tests in `test/tools.test.ts`
