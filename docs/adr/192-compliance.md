...
Checks creatives against platform policies and custom compliance rules. This capability
...
Implement `compliance` as a creative library in `src/lib/creative/compliance.ts`
that:
...
- Exposes API routes under `/api/creative/compliance`

### API

- `GET /api/creative/compliance` — returns credit cost and schema
- `POST /api/creative/compliance` — executes the compliance
...
- Covered by unit tests in `test/compliance.test.ts`
