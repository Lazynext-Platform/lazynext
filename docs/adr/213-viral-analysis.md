...
Analyzes content for virality factors, emotional journey, and shareability. This capability
...
Implement `viral-analysis` as a creative library in `src/lib/creative/viral-analysis.ts`
that:
...
- Falls back to deterministic dry-run output when Atlas is unavailable or
  API key is missing
- Costs 6 credits per generation
- Uses existing auth, credit deduction/refund, and `safeError` conventions
- Exposes API routes under `/api/creative/viral-analysis`

### API

- `GET /api/creative/viral-analysis` — returns credit cost and schema
- `POST /api/creative/viral-analysis` — executes the viral analysis

## Consequences

- Adds a 6-credit cost to user accounts when used
...
- Covered by unit tests in `test/viral-analysis.test.ts`
