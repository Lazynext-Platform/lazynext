...
Centralized prompt templates for all creative AI generation workflows. This capability
...
Implement `prompts` as a creative library in `src/lib/creative/prompts.ts`
that:
...
- Exposes API routes under `/api/creative/prompts`

### API

- `GET /api/creative/prompts` — returns credit cost and schema
- `POST /api/creative/prompts` — executes the creative prompts
...
- Covered by unit tests in `test/prompts.test.ts`
