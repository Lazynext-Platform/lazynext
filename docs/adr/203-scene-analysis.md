...
Analyzes video scenes for visual composition, pacing, and emotional beats. This capability
...
Implement `scene-analysis` as a creative library in `src/lib/creative/scene-analysis.ts`
that:
...
- Exposes API routes under `/api/creative/scene-analysis`

### API

- `GET /api/creative/scene-analysis` — returns credit cost and schema
- `POST /api/creative/scene-analysis` — executes the scene analysis
...
- Covered by unit tests in `test/scene-analysis.test.ts`
